...
$(window).load(function() {
    ...
    function save_options() {
        storage.set({ "_userName": document.getElementById('user').value });
    }

    document.getElementById('save').addEventListener('click',
        save_options);

    storage.get("_userName",
        function(result) {
            document.getElementById('user').value = result._userName

            var userName = document.getElementById('user').value;
            console.log(userName);

            // get most recent song
            lastfm.user.getRecentTracks({ limit: 1, user: userName }, {
